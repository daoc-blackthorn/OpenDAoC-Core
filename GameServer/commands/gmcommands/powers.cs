    ...
    public class PowersCommandHandler : AbstractCommandHandler, ICommandHandler
    {
        ...
        public void OnCommand(GameClient client, string[] args)
        {
            ...
            switch (args[1].ToLowerInvariant())
            {
                ...
                case "reset":
                case "alloff":
                    client.Player.DisableAllPowers();
        ...
        {
            if (!TryGetToggleState(client, args, client.Player.GodModePowerEnabled, out bool enabled))
                return;

            client.Player.SetGodModePower(enabled);
        ...
        {
            if (!TryGetToggleState(client, args, client.Player.AttackablePowerEnabled, out bool enabled))
                return;

            client.Player.SetAttackablePower(enabled);
            string detail = enabled
        ...
        private void ToggleDamageBoost(GameClient client, string[] args)
        {
            ...
            if (args.Length == 2)
            {
                if (player.DamageBoostPowerEnabled)
                {
                    player.DisableDamageBoostPower();
                    DisplayPowerState(client, "Damage boost", false, $"Effectiveness restored to {player.Effectiveness:0.0}.");
                }
                else
            ...
            {
                EnableDamageBoost(client, player.DamageBoostPowerEnabled ? player.DamageBoostPowerMultiplier : DefaultDamageBoost);
                return;
            }
        ...
        {
            client.Player.EnableDamageBoostPower(multiplier);
            DisplayPowerState(client, "Damage boost", true, $"Player effectiveness is {multiplier:0.0}.");
        }
        ...
        private void DisplayStatus(GameClient client)
        {
            ...
            if (player.DamageBoostPowerEnabled)
            {
                DisplayMessage(client, $"  - Damage boost ({player.DamageBoostPowerMultiplier:0.0} effectiveness)");
                anyEnabled = true;
            }

            if (player.AttackablePowerEnabled)
            {
        ...
        private void DisplayHelp(GameClient client)
        {
            ...
            DisplayMessage(client, "  /powers damageboost [on|off|multiplier] - set effectiveness (default 10.0)");
